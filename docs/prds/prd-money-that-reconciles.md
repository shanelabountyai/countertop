...
- **(Product — blocking P0-1)** Does `paymentState` stay as the truth with new enum values, or become a derived cache over a payment event stream? The event stream is the honest model and is what makes the settlement query possible; the enum is one column every reader already uses and is cheap. **A human has to pick; the whole PRD's data model forks here.**
- **(Product)** The operator wants comps because "making it right is the counter's job, not a payments feature". The master PRD's Non-Goal says no real payment processing. These do not actually conflict — a comp is a recorded fact, not a charge — but somebody has to agree that reading in writing, because it is one sentence away from being read as a Non-Goal breach.
- **(Ops — blocking, arguably)** The systems reviewer says a comp control must not ship without a name attached to it, because that is the one place a restaurant cannot ship anonymously. The operator agrees from the other side ("who comped the torta"). **Does P0-3 wait for staff identity, or ship with `actor: 'staff'` and gain a name later?** Shipping first is faster; shipping first is also how an untraceable cash control ends up in production.
...
- **(Builder)** Should a `remake` create a genuinely new order (new `seq`, new ticket, cooked normally) linked to the original, or an adjustment on the original? The kitchen needs a ticket; the report needs a link. These pull in different directions.
