...
- [ ] A **CHECK constraint** enforces `loyaltyExpiryDays <= retentionDays`. It is impossible to configure a program that needs PII kept longer than the retention policy allows. **Hard dependency: PRD 6 C-091 lands the retention setting first**
...
- **C-105 — Expiry, and the forget** — P0-5: the expiry sweep, the `loyaltyExpiryDays <= retentionDays` CHECK, and extending PRD 6's forget path over the loyalty tables with the byte-identical report assertion. **One session.** **Hand-written migration** for the CHECK. Blocked on `C-091`.
