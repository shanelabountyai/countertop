...
| 4 | [`prd-menu-under-pressure.md`](prd-menu-under-pressure.md) | an 86 that names what it hits; search on the availability board; a bulk 86 over a selection (the category grain was resolved away at C-109); dayparts (P1); effective-dated prices (P1); stations (P1) | **all three** (DX 7 · OPS 7 · SYS 11) | M–L — 6 sessions; P0 needs no migration at all | Three lenses, and the P0 half is the cheapest work in this whole set — the affected-item warning needs no migration and no new query, because `ItemModifierGroup` already holds the join. Ranked below money because the daypart half is the expensive part and can wait. |
...
| 7 | Is a remake a real second order, or an adjustment kind on the original? *(PRD 3's own Builder question: "the kitchen needs a ticket; the report needs a link")* | **A real second order** — new `seq`, real ticket, linked by `OrderEvent.relatedOrderId`, comped in full at creation, and skipped by the report. | The two needs do not pull evenly. Either shape gives the report its number; only this one gives the kitchen a ticket, and a remake nobody is told to cook is the transcription failure the product exists to kill. The comp stops a $13.75-due ticket appearing for money nobody will collect, and reuses C-065. The report exclusion is load-bearing: without it the remake books a second sale and an onions attach on an order that said NO onions — the bug this PRD opens with. *(PRD 3 P0-3, C-066.)* |

### Decisions taken later — 2026-09-02

| # | Question | Decision | Why |
|---|---|---|---|
| 7 | Is a remake a real second order, or an adjustment kind on the original? | **A real second order** — new `seq`, real ticket, linked by `OrderEvent.relatedOrderId`, comped in full, skipped by the report. | Recorded in full in the PRD 3 table above. *(C-066.)* |
| 8 | Does the master PRD's **loyalty Non-Goal** get lifted? | **Lifted, by the owner**, on an instruction to build the program out for customer and counter. | `prd-loyalty.md`'s own recommendation was to shelve it, and that recommendation was read and overruled — which is the owner's call. Recorded rather than softened: this is the one PRD in the set with zero lens consensus, and it now ranks by decision rather than by evidence. *(PRD 7, C-100 onward.)* |
...
- **Phone verification / SMS one-time codes.** ~~The prerequisite for self-serve redemption at checkout.~~ **Shipped as C-115**, as a stub whose boundary is a seam (`SmsVerifyProvider`) rather than an env flag — the stub returns the code it was asked to send, a real provider would return `null`. A REAL carrier is still master PRD P2 and still unbuilt; what is closed is the *verification* gap, not the "real SMS" one.
- ~~**Self-serve redemption at checkout.**~~ **Shipped, C-115 → C-118, and PRD 7 P1-1 is live.** The mechanism took three sessions (C-115 verification, C-116 the bearer token, C-117 `Order.discountCents` and the `subtotal − discount` tax base) and the caller took a fourth, unphased one (C-118: the checkout control, the transactional `redeem`, and the settlement that hands a reward back when the order it was spent on dies). **The two decisions C-118 had to take rather than code around** are recorded in the PRD's own entry: checkout and the counter agree by construction because checkout is always first, and the sales report gained a fourth money column rather than netting rewards into net sales.
