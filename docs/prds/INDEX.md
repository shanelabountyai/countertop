...
- **Self-serve redemption at checkout.** PRD 7 P1-1's mechanism shipped over three sessions (C-115 verification, C-116 the bearer token, C-117 `Order.discountCents` and the `subtotal − discount` tax base) — every piece a reward needs to land honestly before tax now exists. What's still missing is the fourth, unphased piece: a checkout control that actually calls `planRedemption`, computes a `discountCents`, and hands it to `placeOrder`. No copy renders and no button exists until that ships.
