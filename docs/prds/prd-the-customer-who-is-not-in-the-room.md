...
**P0-4: The menu says what the food is** *(DX 4)* — **shipped, C-080**
- [x] The menu row renders `MenuItem.description` under the name when present — the column was already in the schema and read by nothing. It renders INSIDE the tap target, so it is in the link's accessible name too
- [x] A sticky category strip jumps to each `<section>` — plain in-page anchors plus `scroll-mt`, no JavaScript
- [x] The composer header renders the description too
- [x] Descriptions are editable in the existing menu editor, phone-viewport, per P0-13 — straight-save, no confirm panel, blank stores NULL
...
- **P1-1: A way back to your own order** *(DX 10)* — **shipped, C-082** — placement appends the status token to a `recent-orders` cookie (the same mechanism the cart already uses), and `/menu` shows a strip: "Your order #005 is cooking — track it", filtered to non-terminal orders with the one status module's own `isTerminal`. No enumeration surface, because the token is still the only key and nothing is looked up by number. **This deliberately revisited a recorded decision**: the WRITEUP records "lose the link and the order is unreachable from the customer side (C-014)" with "the counter staff" as the recovery path — which is a person interrupting the expo mid-rush, the exact interruption the product exists to eliminate. Re-opened 2026-09-09 (see Open Questions) rather than waiting on P1-3's unbuilt SMS.
...
- **P1-3: Photos** ✅ *(C-162)* *(DX 4)* — the master PRD's User Story 1 asks for them and they are in neither Non-Goals nor the WRITEUP's caveats. M, plus a migration and an asset story.
...
- **(RESOLVED 2026-09-09 — re-opened; build it now, ahead of P1-3's SMS.)** P1-1 revisited the decision the WRITEUP recorded deliberately — the recorded recovery path (walk to the counter) is the exact interruption the product exists to eliminate, and the cost of the cheap fix (one cookie, no new lookup surface) is low enough that waiting for SMS was not worth the wait. Built as C-082.
...
- **(RESOLVED 2026-09-01 — keep it, label it, add a deletion path.)** Collection continues. *Why this way:* the premise that it feeds only an unbuilt feature is wrong — it has one live reader, `apps/web/app/kitchen/orders/[id]/page.tsx:53`, which shows it on the staff receipt so a customer whose order was wrong or uncollected can be called. That passes the "use it or stop asking" test, if barely. So the work is not to stop collecting: the checkout form must say what the number is for, and the number needs the retention limit and deletion path it currently lacks (kept forever, searchable forever, no way to erase). Those land in PRD 6 alongside retention. Not re-opened.
...
- **C-082 — A way back to your own order** ✅ — P1-1, the Open Question above re-opened and resolved 2026-09-09.
