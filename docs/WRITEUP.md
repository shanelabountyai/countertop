...
- **The program has no switch** (C-101). `loyaltyEnabled` is a column with no operator control; enabling it means a database write, and the e2e fixture does exactly that. Deliberate scope: the toggle belongs on C-106's loyalty screen, and a checkbox on the service settings form now would be a control for a feature with no numbers behind it. Recorded because "off by default" is doing real work in this project — every other spec in the suite runs against the disabled state — and a toggle wired in casually is how that stops being true.
